...
def main():
    ...
    ctrl_pts_offset_srf = layer_2(ctrl_pts_offset)
    base_surf_pts = base_surf.detach().cpu().numpy().squeeze()
    ctrl_pts_offset_srf_pts = ctrl_pts_offset_srf.detach().cpu().numpy().squeeze()
    ...
    for i in pbar:
        ...
        if element_array.size > 1:
            ...
            with torch.no_grad():
                ...
                for k in range(edge_ctrl_pts_map.shape[0]):
                    for h in range(edge_ctrl_pts_map.shape[1]):
                        if edge_ctrl_pts_map[k][h][0] != 0:
                            temp = torch.zeros(4)

                            temp += inp_ctrl_pts.data[k][h % num_ctrl_pts2][h // num_ctrl_pts2]
                            for j in range(edge_ctrl_pts_map[k][h][0]):
                                elem = edge_ctrl_pts_map[k][h][2 * j + 1]
                                pts = edge_ctrl_pts_map[k][h][2 * j + 2]
        ...
        if i + 1 % 50000 == 0:
            ...
            surf4 = ax.plot_surface(ctrl_pts_offset_srf_pts[0, :, :, 0], ctrl_pts_offset_srf_pts[0, :, :, 1], ctrl_pts_offset_srf_pts[0, :, :, 2], color='yellow')
            surf4 = ax.plot_surface(ctrl_pts_offset_srf_pts[1, :, :, 0], ctrl_pts_offset_srf_pts[1, :, :, 1], ctrl_pts_offset_srf_pts[1, :, :, 2], color='yellow')
            # ax.set_zlim(-1,3)
            # ax.set_xlim(-1,4)
            # ax.set_ylim(-2,2)
            ax.azim = -90
    ...
    # surf4 = ax.plot_surface(ctrl_pts_offset_srf_pts[0, :, :, 0], ctrl_pts_offset_srf_pts[0, :, :, 1],
    #                         ctrl_pts_offset_srf_pts[0, :, :, 2], color='yellow')
    # surf4 = ax.plot_surface(ctrl_pts_offset_srf_pts[1, :, :, 0], ctrl_pts_offset_srf_pts[1, :, :, 1],
    #                         ctrl_pts_offset_srf_pts[1, :, :, 2], color='yellow')
    ax.set_box_aspect([3, 1, 3])
    # ax.set_zlim(-1,3)
    # ax.set_xlim(-1,4)
    # ax.set_ylim(-2,2)
    ...
    loss_cp_pc, _ = chamfer_distance(ctrl_pts_offset_srf.view(element_array.size, eval_pts_size_HD * eval_pts_size_HD, 3),
                                     target_2)
    ...
    loss_cp_Base, _ = chamfer_distance(
        ctrl_pts_offset_srf.view(element_array.size, eval_pts_size_HD * eval_pts_size_HD, 3),
    ...
    loss_cp_pred, _ = chamfer_distance(
        ctrl_pts_offset_srf.view(element_array.size, eval_pts_size_HD * eval_pts_size_HD, 3), out_2)
